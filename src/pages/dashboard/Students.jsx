...
//         toast.success(
//           `✅ Student added! Login: ${formData.parent_email} | Password: ${password}`,
//           { duration: 10000 }
//         )
//       } else {
...
const Students = () => {
  ...
  // ── Add Student (COMPLETELY UPDATED VERSION) ─────────────────────
  const handleAdd = async (formData) => {
    setFormLoading(true)
    try {
      ...
      // Step 1 — Add student to students table with proper field mapping
      const studentData = {
        ...
        parent_whatsapp: formData.parent_whatsapp || formData.parent_phone,
      ...
      if (formData.parent_email && formData.parent_email.trim() !== '') {
        // Map the data to what the edge function expects
        const userAccountData = {
          email: formData.parent_email,           // Required
          password: password,                      // Required
          role: 'student',                         // Required
          full_name: formData.parent_name,         // Required - use parent's name
          phone: formData.parent_phone,            // Optional
          student_id: newStudent.id,               // Link to the new student
          staff_id: null,
        }
        
        console.log('Calling createUserAccount with:', userAccountData)
        
        const result = await createUserAccount(userAccountData)
        
        console.log('createUserAccount result:', result)

        // Show credentials to admin with better formatting
        toast.success(
          `✅ Student Added Successfully!\n\n📧 Email: ${formData.parent_email}\n🔑 Password: ${password}\n\n⚠️ Please save these credentials and share with parent.`,
          { duration: 15000 } // 15 seconds for longer visibility
        )
      } else {
        toast.success('Student added successfully! (No email provided for portal access)')
      }
      ...
      console.error('=== Error in handleAdd ===')
