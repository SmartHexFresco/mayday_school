...
// Single source of truth for the two numbers. Call goes to the normal
// line, WhatsApp (both the contact card link and the form submission)
// goes to the dedicated WhatsApp line.
const CALL_NUMBER = '+234 802 271 1280'
const WHATSAPP_NUMBER = '+234 803 334 3041'

// wa.me needs digits only, no "+" and no spaces.
...
const contactInfo = [
  ...
  },
  {
    ...
    action: (line) => `https://wa.me/${toWhatsAppDigits(line)}`,
    color: 'bg-yellow-50 text-yellow-700',
  },
  {
    icon: Mail,
  ...
  },
  {
    icon: MapPin,
    label: 'Visit Us',
    lines: ['Plot 427 Bishop Shanahan Road,', 'near New Timber Market, GRA, Enugu 400103, Nigeria'],
    action: () =>
      'https://www.google.com/maps/place//data=!4m2!3m1!1s0x1044a311598314b1:0xc481ce57ded232d6?entry=gemini&utm_source=gemini&utm_campaign=gem-default',
    color: 'bg-red-50 text-red-700',
  },
]

// Turns the validated form fields into a readable WhatsApp message.
const buildWhatsAppMessage = ({ full_name, email, phone, subject, message }) => {
  const lines = [
...
const Contact = () => {
  ...
  const onSubmit = async (formData) => {
    setLoading(true)
    try {
      // Keep a record in the database. If this fails we still want the
      // person's message to reach the school, so it doesn't block the
      // WhatsApp handoff below.
      const { error } = await supabase.from('contact_messages').insert([{ ...formData, is_read: false }])
      if (error) throw error
    } catch (error) {
      toast.error('Could not save your message, but we\u2019re sending it via WhatsApp.')
    } finally {
      setLoading(false)
    }

    // Hand the message straight to WhatsApp, pre-filled with everything
    // the person typed (name, email, phone, subject, message).
    window.open(buildWhatsAppUrl(formData), '_blank', 'noopener,noreferrer')

    toast.success('Message ready — continue in WhatsApp!')
    setSubmitted(true)
    reset()
  }
              ...
              <div>
                ...
                {lines.map((line, i) => (
                  <a
                    key={i}
                    href={action(line)}
                    target={['WhatsApp', 'Visit Us'].includes(label) ? '_blank' : undefined}
                    rel={['WhatsApp', 'Visit Us'].includes(label) ? 'noopener noreferrer' : undefined}
                    className="block text-gray-500 hover:text-blue-700 transition-colors text-sm"
                  >
                    {line}
                  </a>
                ))}
            ...
            <div className="relative z-10">
              ...
              <div className="flex gap-4">
                <a
                  ...
                  className="p-3 bg-white/10 rounded-full hover:bg-yellow-500 transition-all cursor-pointer"
                >
                  <MessageCircle size={20} />
                </a>
                <a
          ...
          {submitted ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white p-12 rounded-3xl shadow-xl border border-blue-50 text-center h-full flex flex-col justify-center items-center">
              ...
              <p className="text-gray-500 mb-8">
                We\u2019ve opened WhatsApp with your message ready to send. If it didn\u2019t open automatically,
            ...
            >
              ...
              <p className="text-center text-xs text-gray-400">
                Sending this form will open WhatsApp with your details filled in, ready to send to our team.
              </p>
            </motion.form>
          )}
        </div>
