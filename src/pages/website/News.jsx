...
import { useState, useEffect } from 'react'
//import { Plus, Trash, Search, Megaphone, Calendar, X, Info } from 'lucide-react'
import { Plus, Trash, Search, Megaphone, Calendar, X, Info, Star } from 'lucide-react'
import { motion, AnimatePresence } from 'framer-motion'
...
const News = () => {
  ...
  const fetchNews = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('announcements')
        .select('*')
        .eq('is_published', true) // Only show published announcements
  ...
  const handleAddNews = async (e) => {
    ...
    // Get current user (assuming you have auth set up)
    ...
    const announcementData = {
      ...
      published_at: new Date().toISOString(),
      is_published: true,
      is_pinned: newNews.is_pinned || false,
      author_id: user?.id || null, // Optional: track who created it
    }
    
    console.log('Attempting to insert:', announcementData)
  ...
  const handleTogglePin = async (id, currentPinnedStatus) => {
    try {
      const { error } = await supabase
        .from('announcements')
      ...
      setNews(news.map(item => 
        item.id === id ? { ...item, is_pinned: !currentPinnedStatus } : item
      ))
      
      toast.success(currentPinnedStatus ? 'Unpinned announcement' : 'Pinned announcement')
    } catch (error) {
                ...
                <AnimatePresence mode='popLayout'>
                  {pinnedNews.map((n) => (
                    <motion.div
                      ...
                      animate={{ opacity: 1, scale: 1 }}
                      exit={{ opacity: 0, scale: 0.9 }}
                      className="bg-yellow-50 border border-yellow-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all group"
                    >
                      <div className="flex items-start justify-between">
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <Star size={14} className="fill-yellow-500 text-yellow-500" />
                            <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${categoryStyles[n.category] || categoryStyles.General}`}>
                              {n.category || 'General'}
                            </span>
                          </div>
                          <h2 className="font-bold text-xl text-gray-800 pt-2 group-hover:text-blue-700 transition-colors">
                        ...
                        <div className="flex gap-2">
                          <button 
                            ...
                            className="p-2 text-yellow-600 hover:text-yellow-700 rounded-lg transition-colors"
                            title="Unpin"
                          >
                          ...
                          <button 
                            ...
                            className="p-2 text-gray-300 hover:text-red-600 rounded-lg transition-colors"
                          >
                            <Trash size={18} />
                          </button>
                        </div>
                      ...
                      <div className="flex items-center gap-4 mt-6 pt-4 border-t border-yellow-100">
                        <div className="flex items-center gap-1.5 text-gray-400 text-xs font-medium uppercase">
                          <Calendar size={14} />
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex gap-2">
                      <button 
                        onClick={() => handleTogglePin(n.id, n.is_pinned)} 
                        className="p-2 text-gray-300 hover:text-yellow-600 rounded-lg transition-colors"
          ...
          {filteredNews.length === 0 && !loading && (
            ...
            >
              ...
              <p className="text-gray-500 mt-1 px-4">
                {search ? 'No announcements matching your search criteria.' : 'Click "Add Announcement" to create your first post.'}
              </p>
            </motion.div>
          )}
        </div>
          ...
          {showModal && (
            <div className="fixed inset-0 z-60 flex items-center justify-center p-4">
              ...
              <motion.div 
                ...
                exit={{ y: 50, opacity: 0 }} 
                className="bg-white rounded-3xl w-full max-w-lg shadow-2xl relative z-10 p-8"
              >
                <div className="flex justify-between items-center mb-6">
                ...
                <form onSubmit={handleAddNews} className="space-y-5">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      id="is_pinned"
                      checked={newNews.is_pinned}
                      onChange={(e) => setNewNews({ ...newNews, is_pinned: e.target.checked })}
                  ...
                  <button 
                    type="submit" 
                    className="w-full bg-blue-700 text-white py-4 rounded-2xl font-bold text-lg hover:bg-blue-800 transition-all shadow-xl"
                  >
                    Publish Announcement
                  </button>
                </form>
